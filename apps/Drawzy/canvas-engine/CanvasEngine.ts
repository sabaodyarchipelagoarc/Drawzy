...
export class CanvasEngine {
  ...
  private handleTexty(e: MouseEvent) {
    ...
    textarea.classList.add("Drawzy-texty");
    ...
    const DrawzyContainer = document.querySelector(
      ".Drawzy-textEditorContainer"
    );

    if (DrawzyContainer) {
      DrawzyContainer.appendChild(textarea);
    ...
    const save = () => {
      ...
      if (DrawzyContainer?.contains(textarea)) {
        DrawzyContainer.removeChild(textarea);
        if (span && document.body.contains(span)) {
          document.body.removeChild(span);
        }
      }
